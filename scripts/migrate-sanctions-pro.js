...
async function runSanctionsProMigration() {
  try {
    console.log('🚀 Démarrage de la migration de la table tbl_sanctions_pro...\n');
    ...
    // Lire le fichier SQL
    const sqlFilePath = path.join(__dirname, '../../database/create_tbl_sanctions_pro.sql');
    ...
    const [results] = await sequelize.query(`
      ...
      AND table_name = 'tbl_sanctions_pro'
    `);
    ...
    if (results[0].count > 0) {
      ...
      const [indexes] = await sequelize.query(`
        SHOW INDEXES FROM tbl_sanctions_pro
      `);
      ...
      throw new Error('La table tbl_sanctions_pro n\'a pas été créée');
    }
