...
async function runMigration() {
  try {
    console.log('🚀 Migration table titres et permis mines (tbl_titres_permis_mines)...\n');

    await sequelize.authenticate();
    console.log('✅ Connexion à la base de données réussie\n');

    const sqlPath = path.join(__dirname, '../database/create_tbl_titres_permis_mines.sql');
    ...
    const [t] = await sequelize.query(`
      ...
      WHERE table_schema = DATABASE() AND table_name = 'tbl_titres_permis_mines'
    `);
    if (t[0].c > 0) console.log('✅ tbl_titres_permis_mines créée avec succès.');
