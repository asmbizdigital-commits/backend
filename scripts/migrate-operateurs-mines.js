...
async function runMigration() {
  try {
    console.log('🚀 Migration table opérateurs mines (tbl_operateurs_mines)...\n');

    await sequelize.authenticate();
    console.log('✅ Connexion à la base de données réussie\n');

    const sqlPath = path.join(__dirname, '../../database/create_tbl_operateurs_mines.sql');
    ...
    const [t] = await sequelize.query(`
      ...
      WHERE table_schema = DATABASE() AND table_name = 'tbl_operateurs_mines'
    `);
    if (t[0].c > 0) console.log('✅ tbl_operateurs_mines créée avec succès.');
