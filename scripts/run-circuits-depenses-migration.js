...
require('dotenv').config({ path: path.join(__dirname, '../../.env') });
process.env.DB_NAME = process.env.DB_NAME || 'hotel_beatrice';

const { sequelize } = require('../server/config/database');

async function runMigration() {
  try {
    console.log('🚀 Migration table circuit dépenses (tbl_circuits_depenses)...\n');

    await sequelize.authenticate();
    console.log('✅ Connexion à la base de données réussie\n');

    const sqlPath = path.join(__dirname, '../database/create_tbl_circuits_depenses.sql');
    ...
    const [rows] = await sequelize.query(`
      ...
      WHERE table_schema = DATABASE() AND table_name = 'tbl_circuits_depenses'
    `);
    if (rows[0].c > 0) console.log('✅ tbl_circuits_depenses créée avec succès.');
