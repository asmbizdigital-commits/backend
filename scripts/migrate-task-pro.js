...
async function runTaskProMigration() {
  try {
    console.log('🚀 Démarrage de la migration de la table tbl_task_pro...\n');
    ...
    // Lire le fichier SQL
    const sqlFilePath = path.join(__dirname, '../database/create_tbl_task_pro.sql');
    ...
    const [results] = await sequelize.query(`
      ...
      AND table_name = 'tbl_task_pro'
    `);

    if (results[0].count > 0) {
      console.log('✅ Table tbl_task_pro créée avec succès !\n');
      
      // Afficher la structure de la table
      const [columns] = await sequelize.query(`
        DESCRIBE tbl_task_pro
      `);
      ...
      throw new Error('La table tbl_task_pro n\'a pas été créée');
    }
