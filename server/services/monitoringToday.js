/**
 * Agrégation « monitoring du jour » (phase test / bureau).
 */
...
/**
 * @param {{ date?: Date|string, bureauId?: number|null }} options
 */
async function buildMonitoringTodayPayload(options = {}) {
  const dayParam = options.date ? new Date(options.date) : new Date();
  ...
  const bureauFilter =
    options.bureauId != null && Number.isFinite(Number(options.bureauId)) && Number(options.bureauId) > 0
      ? Number(options.bureauId)
      : null;
  ...
  const connIds = [
    ...new Set(
      rows
        .map((r) => Number(r.connaissementId))
        .filter((id) => Number.isFinite(id) && id > 0)
    )
  ];
  const geoMap = await loadGeoByConnaissementIds(connIds);

  const rowsScoped = bureauFilter
    ? rows.filter((r) => {
        const geo = geoMap.get(String(r.connaissementId));
        return Number(geo?.bureauId) === bureauFilter;
      })
    : rows;

  const eventsAll = rowsScoped.map(serializeEvent);
